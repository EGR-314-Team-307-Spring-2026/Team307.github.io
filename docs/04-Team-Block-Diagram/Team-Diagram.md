...
The block diagram was broken down by the team into major **subsystems**, this makes it easier to provide the right user experience and meet product requirements through good quality and clear requirements for **power distribution, signal flow, control processing, and inter-module communication**, making integration easier as well as debugging and testing. The **power subsystem** displays how the DC input and voltage regulation provide stable power to the electronics and distribute a shared ground across all modules. The **control subsystem**, which focuses on every team member’s microcontroller, performs input processing and outputs control over that module. The **sensor/input subsystems** are those components that gather data or user input and send signals to the controller. The **actuator/output subsystems** represent devices like LEDs or other indicators which respond to instructions that are given by the controller. Finally, the **communication and connector subsystems** connect the team modules together, and signals as well as power are transferred to the entire system. As a team, we decided that pin 8 would be shared ground so ensure stable connection throughout the communication when adding together all the components. Furthermore, shared power is through pin 1. The team decided to have pin 3 as the uart reciever pin (RX), and pin 4 is the uart transmitter (TX).
